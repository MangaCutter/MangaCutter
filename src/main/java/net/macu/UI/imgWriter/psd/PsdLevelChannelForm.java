package net.macu.UI.imgWriter.psd;
...
public class PsdLevelChannelForm {
                ...
                new Thread(() -> {
                    ...
                    try {
                        ...
                        if (val >= from && val <= to) {
                            ...
                            ViewManager.showMessageDialog("UI.psd.PsdLevelChannelForm.bad_range", null, from, to);
                        }
                    } catch (NumberFormatException ex) {
                        ViewManager.showMessageDialog("UI.psd.PsdLevelChannelForm.nan", null, text);
                    }
                    inputRangeFloorTextField.setText(String.valueOf(inputRangeFloor));
                }).start();
            }
                ...
                new Thread(() -> {
                    ...
                    if (text.length != 2) {
                        ...
                        try {
                            ...
                            if (numerator >= 0 && numerator <= 9 && denominator >= 0 && denominator <= 99 &&
                                    numerator * 10 + denominator >= 10) {
                                gamma = numerator * 100 + denominator;
                                return;
                            } else {
                                ViewManager.showMessageDialog("UI.psd.PsdLevelChannelForm.gamma_bad_range", null);
                            }
                        } catch (NumberFormatException ex) {
                            ViewManager.showMessageDialog("UI.psd.PsdLevelChannelForm.nan", null, gammaTextField.getText());
                        }
                    }
                    setGammaTextField(gamma);
                }).start();
    ...
    public short getInputFloor() {
        return (short) inputRangeFloor;
    }

    public short getInputCeiling() {
        return (short) inputRangeCeiling;
    }

    public short getOutputFloor() {
        return (short) outputRangeFloor;
    }

    public short getOutputCeiling() {
        return (short) outputRangeCeiling;
    }
